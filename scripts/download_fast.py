...
download_fast.py
================
Ultra-fast multi-threaded CLI downloader for ARTPARK-IISc/Vaani.
Downloads 8 Parquet files in parallel with direct HTTP streaming chunks
and automatic retries for maximum speed without data loss.
"""
...
def main():
    ...
    print(f"[download_fast] Output directory: {OUTPUT_DIR}", flush=True)
    print(f"[download_fast] Fetching file list for {len(CONFIGS)} West Bengal districts...", flush=True)
    ...
    print(f"[download_fast] Found {total_count} total parquet shards across 11 districts.", flush=True)
    print(f"[download_fast] Starting 8x parallel high-speed downloads...\n", flush=True)
    ...
    print(f"\n[download_fast] Download Complete! Successfully saved {completed}/{total_count} shards in: {OUTPUT_DIR}", flush=True)
